...
from .. import parsers
from ..triqler import __copyright__, __version__
from . import helpers
...
def parseMqEvidenceFile(mqEvidenceFile, fileInfoList, params):
    ...
    for lineIdx, row in enumerate(reader):
        ...
        if (
            sum(1 for x in intensityChannels if not (x[0] + row[fileCol]) in fileList)
            > 0
        ):
            print(
                "Warning: Could not find %s in the specified file list, skipping row"
                % (
                    [
                        x[0] + row[fileCol]
                        for x in intensityChannels
                        if not (x[0] + row[fileCol]) in fileList
                    ]
                )
            )
            continue
        ...
        if (
            sum(1 for x in intensityChannels if row[x[1]] == "") > 0
            or float(row[scoreCol]) <= 0
        ):
            continue

        for prefix, intensityCol in intensityChannels:
            run, condition, sample, fraction = getFileInfo(
                prefix + row[fileCol],
                fileList,
                fileInfoList,
                row[experimentCol],
            ...
            triqlerRow = parsers.TriqlerInputRow(
                ...
                linkPEP,
                featureClusterIdx,
                np.log(float(row[scoreCol])),
                float(row[intensityCol]),
                row[peptCol],
                proteins,
            )
...
def getFileInfo(
    fileName, fileList, fileInfoList, sampleInEvidence, fractionInEvidence, fractionCol
):
