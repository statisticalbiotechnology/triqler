...
def main():
    ...
    convertQuandenserToTriqler(
        args.file_list_file,
        args.in_file,
        args.psm_files.split(","),
        args.out_file,
        params,
    )
...
def convertQuandenserToTriqler(
    fileListFile, clusterQuantFile, psmsOutputFiles, peptQuantRowFile, params
):
    ...
    if not params["skipNormalization"]:
        ...
        if not os.path.isfile(clusterQuantFileNormalized):
            ...
            normalize.normalizeIntensitiesRtimeBased(
                clusterQuantFile,
                clusterQuantFileNormalized,
                minRunsObservedIn,
                plotScatter=params["plotScatter"],
            )
        else:
            print(
                "Reusing previously generated normalized feature group file:",
                clusterQuantFileNormalized,
                ". Remove this file to redo normalization",
            )
    ...
    printTriqlerInputFile(
        fileInfoList, clusterQuantFile, peptQuantRowFile, specToPeptideMap, params
    )
...
def printTriqlerInputFile(
    fileInfoList, clusterQuantFile, quantRowFile, specToPeptideMap, params
):
    ...
    spectrumToFeatureMatch = (
        dict()
    )  # stores the best peptideQuantRow per (peptide, spectrumIdx)-pair
    for featureClusterIdx, featureCluster in enumerate(
        parsers.parseFeatureClustersFile(clusterQuantFile)
    ):
        ...
        for pc in featureCluster:
            ...
            for peptLinkPEP in pc.peptLinkPEPs.split(","):
                ...
                if (
                    pc.intensity > 0.0
                    and linkPEP < 1.0
                    and (params["retainUnidentified"] or peptide != "NA")
                ):
